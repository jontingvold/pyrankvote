...
def almost_equal(value1, value2, considered_equal_margin):
    return abs(value1 - value2) < considered_equal_margin


def assert_list_almost_equal(test_obj, correct_list, li, considered_equal_margin=0.0001, msg=""):
    test_obj.assertEqual(len(correct_list), len(li), "List should be of same length")

    almost_equal_entries = [almost_equal(correct_list[i], li[i], considered_equal_margin)
